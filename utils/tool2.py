...
def _iter_samples_flat(ds_subset):
    """
    Iterate samples in the coordinate space of the *base* dataset.
    Expect base.samples[i] = (path, class_label_str, distance_str) for RxAgnosticCISDataset.
    """
    base, idxs = _flatten_to_base_and_indices(ds_subset)
    if hasattr(base, "samples"):
        for i in idxs:
            yield base.samples[i]  # (path, label_str, distance_str)
    else:
        # Fallback: call __getitem__ (slower; may load .mat)
        for i in idxs:
            item = base[i]
            if isinstance(item, (list, tuple)) and len(item) >= 3:
                yield (None, item[1], item[2])  # (path?, label, distance)
